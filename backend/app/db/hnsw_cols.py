...
def create_column_embeddings_table():
    with DatabaseConnection() as db:
        query = """
        CREATE TABLE IF NOT EXISTS paper_column_embeddings (
        ...
        index_query = """
        CREATE INDEX IF NOT EXISTS paper_column_embeddings_idx
        ON paper_column_embeddings USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
        """
        db.cursor.execute(index_query)
        db.conn.commit()
        print("✅ Index paper_column_embeddings_idx created successfully.")
...
def insert_column_embeddings():
    with DatabaseConnection() as db:
        ...
        table_name = 'paper'
        csv_file_path = "/Users/raelin/HITS/scripts/paper.csv"
        ...
        insert_query = """
        INSERT INTO paper_column_embeddings (table_name, column_name, embedding)
        VALUES (%s, %s, %s)
        """
